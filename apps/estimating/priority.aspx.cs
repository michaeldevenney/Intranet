...
{    
    ...
    private string CreateDir(Estimate.DirType dirType)
    {
        if (dirType == Estimate.DirType.Exec)
        {
            ...
            string path = "";
            var execPath = ConfigurationManager.AppSettings["ExecFolderPath"];

            if (region == "United States")
            {
                path = Utils.BetweenAandM(project.Substring(0, 1)) ? 
                    execPath + region + @"\A-M\" + txtAddProjNumber.Text + " - " + txtAddProjName.Text :
                    execPath + region + @"\N-Z\" + txtAddProjNumber.Text + " - " + txtAddProjName.Text;
            }
            else
            {
                path = execPath + region + @"\" + txtAddProjNumber.Text + " - " + txtAddProjName.Text;
            }
        ...
        {
            string folderTemplatePath = ConfigurationManager.AppSettings["ProspectFolderTemplate"].ToString();

            string newDir = copyDirectory(folderTemplatePath, Folders.ProspectsPath
    ...
    private void BindControls()
    {
        ...
        string[] regions = Directory.GetDirectories(Folders.ProspectsPath, "*", SearchOption.TopDirectoryOnly);
