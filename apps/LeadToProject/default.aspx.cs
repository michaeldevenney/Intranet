...
public partial class apps_LeadToProject_default : Page
{
    ...
    protected void cmdGo_Click(object sender, EventArgs e)
    {
        ...
        var prospect = new Project(prospectID);
        string projectsDirectory = Folders.ProjectsPath;
        string execDirectory = Folders.ExecPath;
        ...
        if (chkNotification.Checked)
        {
            ...
            User pm = DAL.User.GetUserByID(int.Parse(prospect.PMUserID.ToString()));
            User sales = DAL.User.GetUserByID(int.Parse(prospect.SalespersonID.ToString()));
            ...
            foreach (ListItem chk in chkUsers.Items)
            {
                ...
                if (chk.Selected)
                {
                    User temp = DAL.User.GetUserFromFullName(chk.Text);
            ...
            msg.Body = "The " + prospect.DisplayName + " has been awarded to Veritas and given the number " +
                       prospect.ProjectNumber + ".  Congratulations to " + sales.Name +
                       " on closing the job!  The project has been assigned to " + pm.Name +
                       " from the Project Management group.";

            var smtp = new SmtpClient("ver-sbs-01")
                {
                    Credentials = new NetworkCredential("prospectus", "BringInTheSales1", "Veritas")
                };
